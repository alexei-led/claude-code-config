...
Extract complete requirements through structured Q&A. Produces PRD-quality `REQ-*.md`. Input is an idea text, a doc path, or an existing `REQ-id` to refine.

## Step 1: Load context

- If input is a `REQ-id`: run `specctl show <REQ-id>` and read the file. Interview to refine.
