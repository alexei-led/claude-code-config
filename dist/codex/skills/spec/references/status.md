...
Match the user's request to one of these views:
...
If `.spec/` doesn't exist, tell the user: "No `.spec/` folder. Run the init workflow first." and stop.
...
Run `specctl validate` and `specctl status`. Surface:
...
For each issue, suggest the workflow that resolves it (usually `interview` to enrich, `plan` to add tasks, or manual edit).
