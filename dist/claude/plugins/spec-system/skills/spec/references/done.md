...
If a follow-up task was discovered, create it via the `new` workflow and link it back: `specctl dep add TASK-<new> TASK-<id> --type discovered-from`.

## Discover potentially done tasks

The user wants to find in-progress tasks that look complete based on git evidence.

For each task with `status: todo` or `status: in-progress`:

```bash
# Show modifications to files mentioned in the task
...
The user wants verification to pass before changing status.
