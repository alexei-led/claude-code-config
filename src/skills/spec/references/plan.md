...
Turn a requirement (or idea) into an `EPIC-*.md` + a set of independently grabbable `TASK-*.md` files with dependencies, blockers, and acceptance criteria.
...
- `"<idea text>"` — ask 3–5 quick questions, then plan (for deep requirements, recommend `spec interview` instead)
...
- **REQ id** → `specctl show REQ-<id>` and read the file.
- **EPIC id** → `specctl show EPIC-<id>` — refinement; may add/update tasks.
...
3. TASK-<slug>-3: <title>
   - Size: S
   - Blocked by: none (parallel with task 2)
   - Verifiable: <demo/check/test>
```
