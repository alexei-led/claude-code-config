...
Full development workflow for one task per session, with user control at every step.
...
- `EPIC-id` argument → `specctl ready --epic EPIC-id` and pick the highest-priority ready task.
- Otherwise → `specctl ready` and pick the highest-priority ready task.

If no ready tasks:

```bash
specctl ready   # shows blockers
```
...
Update session step: `specctl session step planning`.
...
Update session step: `specctl session step implementing`.
...
`BASE_COMMIT` is already recorded in `.spec/SESSION.yaml` from `specctl start`.
...
Update session step: `specctl session step testing`.
...
Update session step: `specctl session step completing`.
...
- Ask the user to describe it, then run `spec new task <slug>`.
- Link back: `specctl dep add TASK-<new-slug> TASK-<id> --type discovered-from`.

## Step 7: Review & commit (user choice)

Update session step: `specctl session step reviewing`.
...
`specctl done` clears the session automatically.

```bash
specctl ready --epic EPIC-<id>   # if working on an epic
...
- Dependency-aware selection (`specctl ready` orders by deps and priority).
- Evidence-tracked completion (`specctl done` records what was done).
