...
tools: Task, mcp__perplexity-ask__perplexity_ask, mcp__gemini__gemini, mcp__codex__spawn_agent
model: sonnet
color: magenta
---
...
| ---------- | -------------------------- | ---------------------------------- |
| Codex      | `mcp__codex__spawn_agent`  | Code patterns, implementation      |
| Gemini     | `mcp__gemini__gemini`      | Architecture, design trade-offs    |
| Claude     | claude-reviewer agent      | Fresh perspective, unbiased review |
| Perplexity | `mcp__perplexity-ask__ask` | Industry best practices, research  |

## Execution

**Call all 4 IN PARALLEL in a single message:**

1. `mcp__codex__spawn_agent(prompt: "Review: <topic>")`
