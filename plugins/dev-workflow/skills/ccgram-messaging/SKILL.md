...
description: "Inter-agent messaging via ccgram swarm. Use when communicating with other agents in the same tmux session \u2014 send messages, check inbox, discover peers, broadcast status, reply to requests, or spawn new agents. Activates on: peer messages, inbox, swarm, ccgram, broadcast, agent collaboration, ask another agent."
user-invocable: true
context: fork
allowed-tools:
  - Bash(ccgram msg *)
