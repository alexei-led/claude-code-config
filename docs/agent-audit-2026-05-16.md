...
`src/skills/spec-work/SKILL.md` was repointed name-swap-only during the consolidation pass (Task 7/8 fence), which left its implementation-mode prose contradicting the new role envelopes. This was resolved in a later code-review pass: the Solo-engineer prompt now tells `engineer` to apply and verify (no "return proposals"), and the Implementation-pair block assigns the `reviewer` role to *propose* tests (read-only Proposed Changes contract) with the `engineer` applying the converged implementation and test proposals. `improving-tests` and `testing-e2e/claude/body.md` were reconciled to the same role-gating in the same pass (reviewer Bash sections gated to engineer; testing-e2e spawn prompts made self-contained for the Playwright workflow). One accepted asymmetry remains by design: in pair mode the engineer applies both its own implementation and the reviewer's test proposals — the alternative (both roles read-only, main applies) was a larger UX change and intentionally not pursued.
